...
if ( ! defined( 'ABSPATH' ) ) exit; 
	...
	function wpus_get_order_ids_by_user($user_id){
		global $wpdb;
		...
		$all_posts = $wpdb->get_results("SELECT ID, post_type FROM $wpdb->posts WHERE post_author='$user_id'");
	...
	function wpus_reassign_shop_orders($order_ids, $reassign_user){
        global $wpdb;
        ...
        $reassign_user_email = $reassign_user_obj->user_email;
		//wpus_pree($order_ids);
        if(!empty($order_ids)){
            ...
            $customer_query = "UPDATE $wpdb->postmeta SET `meta_value` = $reassign_user WHERE `meta_key` = '_customer_user' AND `post_id` IN ($order_ids_str)";
            $email_query = "UPDATE $wpdb->postmeta SET `meta_value` = '$reassign_user_email' WHERE `meta_key` = '_billing_email' AND `post_id` IN ($order_ids_str)";
			$post_author_query = "UPDATE $wpdb->posts SET `post_author` = $reassign_user WHERE `ID` IN ($order_ids_str)";
			//wpus_pree($customer_query);wpus_pree($email_query);wpus_pree($post_author_query);
					...
					if($_GET['user_id'] && $_GET['user_id']>0){
						...
						if(!empty($course_meta_keys)){
							foreach($course_meta_keys as $ckey){
								$cq = "SELECT meta_key, meta_value FROM $wpdb->usermeta WHERE user_id=$user_id AND meta_key LIKE '$ckey%'";
								wpus_pre($cq);
